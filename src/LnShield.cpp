...
LnShield::Err_t LnShield::init()
{
    ...
    //digitalWrite(M_PIN_OE, HIGH);   //for Arduino program write
    digitalWrite(M_PIN_OE, LOW);    //output enable
...
LnShield::Err_t LnShield::cmdGetLastInvoice(char *pInvoice, size_t *pLen)
{
    ...
    err = uartSendCmd(CMD_GETLASTINVOICE, 0, 0, (uint8_t *)pInvoice, &recv_len);
    if (err == ENONE) {
        if (recv_len < *pLen) {
            pInvoice[recv_len] = '\0';
            *pLen = recv_len;
